...
## Часть 6: Обновление компонента Task

### 6.1. Проблема с отображением чекбокса

Компонент `Task.tsx` отображает чекбокс для каждой задачи. Сейчас его состояние определяется так:

```typescript
<Checkbox
  checked={task.completed}  // ← Это поле больше не существует!
  onChange={() => onToggle(task.id)}
/>
```

После удаления поля `completed` из интерфейса `Todo`, это вызовет ошибку TypeScript, а в runtime чекбокс всегда будет unchecked.

### 6.2. Решение: проверка columnId

Нужно определить, является ли задача "завершенной", проверяя её `columnId`:
...
**Логика:**
- Если задача в колонке с id "completed" → чекбокс отмечен
- Если в любой другой колонке → чекбокс пустой

**Почему именно 'completed'?**
- Это конвенция для колонки завершенных задач
- Соответствует тому, как мы преобразуем данные API: `completed: true` → `columnId: 'completed'`
- Пользователи ожидают, что чекбокс означает "завершено"

### 6.3. Альтернативный подход для множественных колонок

Если у вас будет много колонок и чекбокс должен иметь смысл только для некоторых:

```typescript
// Например, задача "завершена" только если в колонке "completed" или "archived"
const isCompleted = ['completed', 'archived'].includes(task.columnId);
...
Или можно добавить поле `isCompletedColumn` в интерфейс `Column`:
...
interface Column {
  ...
  title: string;
  order: number;
  isCompletedColumn?: boolean;  // Флаг "завершенной" колонки
}

// В компоненте Task получаем информацию о колонке
const column = columns.find(c => c.id === task.columnId);
<Checkbox checked={column?.isCompletedColumn || false} />
```

Для начала простая проверка `task.columnId === 'completed'` вполне достаточна.

---

## Часть 7: Перемещение задач между колонками

### 7.1. Семантика операции
...
### 7.2. Реализация функции перемещения
...
### 7.3. Обратная совместимость с чекбоксом
...
## Часть 8: Добавление новых колонок

### 8.1. Генерация ID из названия
...
### 8.2. Альтернативные стратегии генерации ID
...
## Часть 9: Персистентность - сохранение колонок

### 9.1. Проблема: колонки теряются при перезагрузке
...
### 9.3. Паттерн: синхронизация state с LocalStorage
...
## Часть 10: Обновление формы создания задачи

### 10.1. Проблема статического Select
...
### 10.2. Динамическая генерация опций
...
### 10.4. Синхронизация с сервером и проблема дублирующихся id

Когда создаем задачу, JSONPlaceholder ожидает `completed: boolean`, но мы работаем с `columnId: string`. Плюс есть критическая проблема с id.

**Проблема:** JSONPlaceholder - это mock API. Когда вы создаете задачу через `POST /todos`, он всегда возвращает одинаковый фейковый id (обычно 201), независимо от того, сколько задач вы создали.

Если вы создадите 3 задачи, все они будут иметь `id: 201`. Когда вы попытаетесь переместить одну задачу с помощью:
...
Все три задачи с `id: 201` переместятся одновременно!

**Решение:** Генерируем уникальный локальный id для каждой новой задачи:
...
const createTask = async () => {
  ...
  // Генерируем уникальный локальный id
  const uniqueId = Date.now() + Math.random();

  // Локально добавляем с columnId и уникальным id
  const taskWithColumn = {
    ...serverTask,
    id: uniqueId,  // Перезаписываем фейковый id с сервера
...
**Почему `Date.now() + Math.random()`?**
- `Date.now()` - миллисекунды с 1970 года, уникальны для каждой миллисекунды
- `+ Math.random()` - добавляет случайную дробную часть
- Вместе дают практически гарантированно уникальное число
- Для production лучше использовать библиотеку UUID, но для локальной разработки достаточно

**Альтернативные стратегии генерации id:**

```typescript
// Инкрементальный id (простой)
const maxId = Math.max(...todos.map(t => t.id), 0);
const uniqueId = maxId + 1;

// Отрицательные id для локальных задач (позволяет различать локальные и серверные)
const uniqueId = -Date.now();

// UUID (требует библиотеку uuid)
import { v4 as uuid } from 'uuid';
const uniqueId = uuid(); // Но id должен быть number, не string
```

**Принцип:** на границе системы (API call) преобразуем данные между форматами. Внутри приложения всегда работаем с `columnId` и локально сгенерированными id.

---

## Часть 11: Архитектурные паттерны и best practices

### 11.1. Разделение concerns (separation of concerns)
...
## Часть 12: Возможные расширения системы
...
### 12.3. Кастомные свойства колонок
